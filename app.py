...
if st.button("Find Matches", type="primary"):
    if query_strings and reference_strings:
        ...
        query_list = [line.strip() for line in query_strings.split("\n") if line.strip()]
        ref_list = [line.strip() for line in reference_strings.split("\n") if line.strip()]
        ...
        if not query_list:
            ...
            for query in query_list:
                ...
                filtered_matches = [(val, score, idx) for (val, score, idx) in top_matches if score >= score_threshold]
                
                # Normalize to 3 entries (fill with "-" if below threshold or not found)
                top_matches_list = list(top_matches) if top_matches else []
                while len(top_matches_list) < 3:
                    top_matches_list.append(("-", 0.0, None))

                (best_val, best_score, _), (second_val, second_score, _), (third_val, third_score, _) = top_matches_list[:3]

                # Apply threshold filter - show "-" if score is below threshold
                best_display = best_val if best_score >= score_threshold else "-"
                best_prob = f"{best_score:.2f}%" if best_score >= score_threshold else "-"
                
                second_display = second_val if (second_val != "-" and second_score >= score_threshold) else "-"
                second_prob = f"{second_score:.2f}%" if (second_val != "-" and second_score >= score_threshold) else "-"
                
                third_display = third_val if (third_val != "-" and third_score >= score_threshold) else "-"
                third_prob = f"{third_score:.2f}%" if (third_val != "-" and third_score >= score_threshold) else "-"
